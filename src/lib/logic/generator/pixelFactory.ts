...
import type { EntityType, TileType } from '$lib/types';
import {
	TILES_PER_PIXEL,
...
export type PixelType = 'accumulator' | 'solar-panel' | 'stone-wall' | 'transparent';
...
const sizes: Record<EntityType, number> = {
	...
	"lightning-collector": TILE_SIZE_ACCUMULATOR
};
...
export const createPixel = (type: PixelType, x: number, y: number, options: Options): Pixel => {
	if (type === 'transparent') {
		return EMPTY_PIXEL;
	}
	...
	if (options.concrete) {
		const tiles =
			type === 'solar-panel'
				? (options.substation || options.lightningRod) && options.roboport
					? TILES_BOTH_ON_SOLAR
					: (options.substation || options.lightningRod)
	...
	for (let yi = 0; yi < TILES_PER_PIXEL; yi += size) {
		for (let xi = 0; xi < TILES_PER_PIXEL; xi += size) {
			if (
				(options.substation && xi < TILE_SIZE_SUBSTATION && yi < TILE_SIZE_SUBSTATION) ||
				(options.lightningRod && xi < TILE_SIZE_SUBSTATION && yi < TILE_SIZE_SUBSTATION) ||
				(options.roboport &&
